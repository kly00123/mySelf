<template>
	<view class="page">
		...
		<view class="price">
			...
			<view class="uni-price-a">{{info.price}}<text style="font-size: 28rpx;">元</text></view>
	...
	export default {
		...
		methods: {
			...
			payOrder() {
				...
				if (_this.payType == 1) {
					if (_this.info.price > _this.item.money) {
						uni.showToast({
							title: "余额不足",
							icon: "none"
						})
				...
				uni.request({
					url: _this.ApiGateWay + "/sale/Member/RechargeOrLevelUp?taskPackageId=" + _this.info.id + "&payType=" + _this.payType + "&taskLevel="+_this.info.taskLevel,
					data: "",
					header: {
