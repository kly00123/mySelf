...
<!-- 							<view style="font-weight:600;color:#FF2A43">任务佣金：
							...
							<view style="font-weight:600;color:#FF2A43;margin-left:128rpx;">已领取/总数：
					...
					success: function(res) {
						if (Helper.CheckData(res)) {
							for(var i=0;i<res.data.list.length;i++){
								if(res.data.list[i].taskType==21){
									 array.push(res.data.list[i]);
									_this.List=array;
									 // _this.endquo = _this.list[i].quota - _this.list[i].usequota;
								}
							}
							_this.taskList = _this.taskList.concat(array);
							_this.total = res.data.total;
							// console.log(_this.List[0].quota-_this.List[0].usequota);
							 // _this.endquo_1=Number(_this.List[i].quota)
							 // _this.endquo_2=Number(_this.List[i].useQuota)
							 // _this.endquo=Number(_this.List[i].quota)-Number(_this.List[i].useQuota)
