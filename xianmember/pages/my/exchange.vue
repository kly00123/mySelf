<template>
	<view class="page">
		...
		<uni-page-head uni-page-head-type="default">
			<div class="uni-page-head" style="background:linear-gradient(90deg,#762f93,#e23b93,#f08300); color: rgb(255, 255, 255);">
				...
				<div class="uni-page-head-bd">
					<div class="uni-page-head__title" style="font-size: 16px; opacity: 1;">兑换记录</div>
				</div>
				<!---->
			</div>
		</uni-page-head>
		...
		<view v-if="0==sendType" id="id-0">
			<view class="textoder-a" v-for="item in firstList">
				<view class="textoder-b">
					<view style="margin-left: 20rpx;color: #ff5500;font-size: 28rpx;" v-if="item.status==0">待发货</view>
				</view>
				<view class="textoder-c">
					...
					<view class="noways">
						...
						<view class="noway">
							<view class="ban">商品名称：{{item.productName}}</view>
							<view>商品数量：{{item.num}}</view>
		...
		<view v-if="1==sendType" id="id-1">
			<view class="textoder-a" v-for="(item,index) in listShiYong" :num-Id="index">
				<view class="textoder-b">
					<view style="margin-left: 20rpx;color: #ff5500;font-size: 28rpx;" v-if="item.status==1">已发货</view>
					<view class="btn" @click.stop="goTaskDetails(item.id,index)">
						确认收货
					</view>
				</view>
				<view class="textoder-c">
		...
		-webkit-line-clamp: 2;">商品名称：{{item.productName}}</view>
							<view>商品数量：{{item.num}}</view>
							<view style="color: #999999;word-wrap: break-word">购买时间：{{item.payTime| formatDate}}</view>
						</view>
					</view>
		...
		<view v-if="2==sendType" id="id-2">
			<view class="textoder-a" v-for="item in lastList">
				<view class="textoder-b">
					<view style="margin-left: 20rpx;color: #ff5500;font-size: 28rpx;" v-if="item.status==2">已签收</view>
				</view>
				<view class="textoder-c">
					<!-- <view>获赞：{{item.quota}}</view> -->
		...
		<view class="nodata" v-if="total==0">
			<view>
				<image src="../../static/nodata.png" style="width: 230upx;height: 164upx;"></image>
			</view>
			暂无商品
		</view>
	...
	export default {
		...
		methods: {
			...
			goDelivery(id) {
				console.log(id);
				uni.navigateTo({
					url: '/pages/trading/order/delivery?id?=' + orderId
				})
			},
			payment(item) {
				uni.navigateTo({
			...
			goTaskDetails(id,index) {
				...
				uni.request({
					url: _this.ApiGateWay + "/sale/shopping/ReceiveShoppingOrder?id=" + id,
					data: '',
					header: {
			...
			getFirstList(status) {
				...
				uni.request({
					url: _this.ApiGateWay + "/sale/shopping/GetshoppingOrderEntitys?status=" + status,
					data: '',
					header: {
						Authorization: _this.token
					},
