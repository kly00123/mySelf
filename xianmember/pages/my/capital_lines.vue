		...
		<view class="one_record">
			...
			<block v-for="(item,index) in rewardList">
				<view class="uni-records">
					...
					<view style="font-size: 30rpx; color:#FF9000;margin-left: 24rpx;" v-if="item.type==121">额度任务</view>				
					<view style="font-size: 30rpx; color:#FF9000;margin-left: 24rpx;" v-if="item.type==126">推荐有效会员</view>
					<view style="font-size: 30rpx; color:#FF9000;margin-left: 24rpx;" v-if="item.type==125">提现</view>
					<view class="date">
						<view style="font-size: 36rpx; color: #333333;" v-if="item.type==5||item.type==10||item.type==15">+{{item.money}}</view>
						<view style="font-size: 36rpx;color:#FF5959;" v-if="item.type==121||item.type==1||item.type==123||item.type==126">+{{item.money}}</view>
						<view style="font-size: 36rpx;color:#FF5959;" v-if="item.type==20">+{{item.money}}</view>
						<view style="font-size: 36rpx; color: #333333;" v-if="item.type==125||item.type==101">-{{item.money}}</view>
