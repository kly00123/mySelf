	...
	<view class="page">
		<uni-page-head uni-page-head-type="default">
			<div class="uni-page-head" style="background:linear-gradient(90deg,#762f93,#e23b93,#f08300); color: rgb(255, 255, 255);">
				...
				<div class="uni-page-head-bd">
					<div class="uni-page-head__title" style="font-size: 16px; opacity: 1;">修改兑换数据</div>
				</div>
				<!---->
			</div>
		</uni-page-head>
		<view class="content">
			<view class="per">
				<view>余额</view>
				<view class="uni-per">
					<input placeholder="渠豆" v-model="info.money" class="input" />
			...
			<view class="per">
				<view>渠豆</view>
				<view class="uni-per">		
					<input placeholder="渠豆" v-model="info.qudou"  class="input" />
					<view class="icons"></view>
				</view>
			</view>
			<view class="per">
				<view>名额(真)</view>
				<view class="uni-per">
					<input placeholder="渠豆" v-if="info.type>0" v-model="info.quota" class="input" />
					<view class="icons"></view>
				</view>
			</view>
			<view class="per">
				<view>备注</view>
				<view class="uni-per">
					<input placeholder="渠豆" v-model="info.remark" class="input" />
					<view class="icons"></view>
				</view>
			</view>
			<view class="per">
				<view>展示名额（假）</view>
				<view class="uni-per">
					<input placeholder="渠豆" v-model="info.showQuota" class="input" />
					<view class="icons"></view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			...
			//console.log(options);
			this.orderId = options.type;
			this.getInfo(this.orderId);
		},
		methods: {
			goIndex() {
				uni.navigateTo({
					url: '/pages/trading/select/sel/update'
				})
			},
			getInfo(id) {
				var _this = this;
				...
				uni.request({
					url: _this.ApiGateWay + '/sale/shopping/GetExchangeQuotaByType?type='+_this.orderId,
					data: "",
					method: 'POST',
					dataType: 'json',
					header: {
			...
			saveData() {
				...
				var parmas = {
					...
					quota: _this.info.quota,
					remark: _this.info.remark,
					showQuota: _this.info.showQuota
				}
				console.log(parmas)
				uni.showLoading({
					title: "请稍候..."
				...
				uni.request({
					url: _this.ApiGateWay + '/sale/shopping/UpdateExchangeQuota',
					data: parmas,
					method: 'POST',
					dataType: 'json',
					header: {
