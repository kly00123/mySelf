	...
	<view class="page">
		...
		<view class="one_box">
			<view class="one_col"   @click="btnClick(0)">
				<view :class="sendType === 0 ? 'one_col_title color_333333' : 'one_col_title'">已付款待发货</view>
			...
			<view class="textoder-a" v-for="item in firstList">
				...
				<view class="textoder-c">
					...
					<view class="noways">
						...
						<view class="noway">
							<view>商品名称：{{item.productName}}</view>
							<view>商品总价：{{item.totalPrice}}</view>
							<view style="color: #999999;word-wrap: break-word">购买时间：{{item.payTime}}</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			getFirstList(status) {
				...
				uni.request({
					url: _this.ApiGateWay + "/sale/shopping/GetshoppingOrderEntitys?status="+1,
					data: '',
					header: {
						Authorization: _this.token
					},
			...
			getShiList(status) {
				...
				uni.request({
					url: _this.ApiGateWay + "/sale/shopping/GetAllShoppingOrderEntitys?status="+status,
					data: '',
					header: {
						Authorization: _this.token
					},
