			...
			<view class="VerticalMain" style="height:calc(100vh - 375upx)">
				<view class="uni-view-content" style="overflow: hidden auto; ">
							...
							<view class="uni-starorder">
								<view class="uni-starorder_A">
									<view class="uni-starorder_b">
										...
										<view>引流客户：{{item.disQuota}}个</view>
					...
					<view class="navPadding" id="id-2" style="display:none;">
						<scroll-view scroll-x class="bg-white nav">
							<view class="flex text-center">
								<view class="cu-item flex-sub" :class="0==tag?'text-orange cur':''" @tap="elseSelect(0)">
									购买套餐
								</view>
								<view class="cu-item flex-sub" :class="1==tag?'text-orange cur':''" @tap="elseSelect(1)">
									使用套餐
								</view>
							</view>
						</scroll-view>
						...
						<view v-else>
	<view class="textoder-a" v-for="item in endlist">
		<view class="textoder-b">
			<view style="margin-left: 20rpx;">{{item.name}}</view>
			<view style="margin-right: 20rpx;color: #F04F43;" v-if="item.state==1">待使用</view>
			<view style="margin-right: 20rpx;color: #999999;" v-else>已使用</view>
			...
			getEnd(taskTypeId){
				...
				uni.request({
					...
					success: function(res) {
						console.log(res);
						if (Helper.CheckData(res)) {
							_this.endlist = res.data.list;
							console.log(_this.endlist)
						}
